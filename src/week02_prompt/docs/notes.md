...
- 在本实验的 25 道标准答案题中, thinking=False 已经能解决大多数题; thinking
  主要救回少数 M1 失败的题, 但代价显著增加。
- 在类似本实验的任务分布下, 全量开启 thinking 可能造成明显的资源浪费。
- 但这不能推广为所有标准答案推理任务都适合默认关闭 thinking; 实际路由仍需要通过
  数据验证哪些请求存在较高的 M1 失败风险。
- 因此 Part 2 的问题不是先验认定"难题该开 thinking", 而是验证 router 能否预测
  **M1 失败风险**, 在质量接近 All M2 的前提下减少不必要的 thinking 成本。
- thinking 的潜在价值场景还包括 Agent 多步规划、长文档综合权衡、代码库级决策等
  复杂任务(留待 Week 12/13 Agent 周体会)。
