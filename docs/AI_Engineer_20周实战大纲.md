...
- [总览](#总览)
- [阶段一：底层原语、成本与本地化（第 1-4 周）](#阶段一底层原语成本与本地化第-1-4-周)
  - [第 1 周：API 原语与 Ollama 本地部署](#第-1-周api-原语与-ollama-本地部署)
  - [第 2 周：Prompt 工程与对话管理](#第-2-周prompt-工程与对话管理)
  - [第 3 周：结构化输出与数据校验](#第-3-周结构化输出与数据校验)
  - [第 4 周：函数调用（Function Calling）纯手写](#第-4-周函数调用function-calling纯手写)
- [阶段二：手搓 RAG 与 Embedding 选型（第 5-8 周）](#阶段二手搓-rag-与-embedding-选型第-5-8-周)
  - [第 5 周：Embedding 与向量检索基础](#第-5-周embedding-与向量检索基础)
  - [第 6 周：文档解析与分块策略](#第-6-周文档解析与分块策略)
  - [第 7 周：向量数据库集成](#第-7-周向量数据库集成)
  - [第 8 周：组装 Naive RAG 并直面缺陷](#第-8-周组装-naive-rag-并直面缺陷)
- [阶段三：框架、进阶 RAG、多模态与 MCP（第 9-12 周）](#阶段三框架进阶-rag多模态与-mcp第-9-12-周)
  - [第 9 周：LangChain 核心抽象与 LCEL](#第-9-周langchain-核心抽象与-lcel)
  - [第 10 周：Advanced RAG（进阶检索策略）](#第-10-周advanced-rag进阶检索策略)
  - [第 11 周：多模态（Multimodal）入门](#第-11-周多模态multimodal入门)
  - [第 12 周：Agent 基础与 MCP 协议](#第-12-周agent-基础与-mcp-协议)
- [阶段四：编排、可观测性与 Harness（第 13-16 周）](#阶段四编排可观测性与-harness第-13-16-周)
  - [第 13 周：LangGraph 状态机编排](#第-13-周langgraph-状态机编排)
  - [第 14 周：可观测性（Observability）— Agent 调试命脉](#第-14-周可观测性observability-agent-调试命脉)
  - [第 15 周：Harness 工程 — 异常捕获与自纠错](#第-15-周harness-工程--异常捕获与自纠错)
  - [第 16 周：自动化评估（Evaluation）](#第-16-周自动化评估evaluation)
- [阶段五：全栈整合与业务交付（第 17-20 周）](#阶段五全栈整合与业务交付第-17-20-周)
  - [第 17 周：FastAPI 构建 AI 后端服务](#第-17-周fastapi-构建-ai-后端服务)
  - [第 18 周：前端 UX 与状态管理](#第-18-周前端-ux-与状态管理)
  - [第 19 周：系统联调与商业级打磨](#第-19-周系统联调与商业级打磨)
  - [第 20 周：简历更新与面试准备](#第-20-周简历更新与面试准备)
- [附录](#附录)
...
> **本周内容：**
> 1. [大模型 API 的 HTTP 本质](#1-大模型-api-的-http-本质)  
> 2. [Token 分词原理与成本计算](#2-token-分词原理与成本计算)  
> 3. [SSE 流式传输（打字机效果的底层原理）](#3-sse-流式传输打字机效果的底层原理)  
> 4. [Ollama 本地部署与开源模型](#4-ollama-本地部署与开源模型)  
> 5. [参数调优实验](#5-参数调优实验)  
...
任务2：综合实验 Top-p、Frequency/Presence Penalty、Stop Sequences，观察各参数效果并总结参数速查表
```
...
- [X]  能用 Python 和 JS 分别实现流式输出
- [X]  本地 Ollama 环境搭建完成，能通过 API 调用本地模型
- [X]  完成参数对比实验，产出参数速查表
...
> **本周内容：**
> 1. [消息角色体系（Messages Array）](#1-消息角色体系messages-array)  
> 2. [多轮对话的实现原理](#2-多轮对话的实现原理)  
> 3. [上下文窗口管理策略](#3-上下文窗口管理策略)  
> 4. [Prompt 核心技巧](#4-prompt-核心技巧)  
> 5. [Prompt 模板化工程](#5-prompt-模板化工程)  
...
- 11_滑动窗口策略：保留 system + 最近 N 轮对话，超出的直接丢弃
- 12_摘要压缩策略：用模型将旧对话压缩为一段摘要，放在 system 之后
- 13_动态 System Prompt 注入：关键信息从历史中提取，注入到 system prompt 模板中
- 14_Pinned Messages（重要消息标记）：给消息加 is_pinned 标记，截断时强制保留
- 15_Token 预估与预算分配：发请求前预估 token 数，为 system/history/output 各分配预算

**进阶预告（本节只需了解概念，后续周会深入）：**

- 结构化状态提取（JSON 记忆池）→ 第3周结构化输出 + 第13周 LangGraph State
- RAG 向量记忆（把历史对话 Embedding 后检索）→ 第5-8周 RAG

**练习任务：**

```
任务1：给 02a 多轮对话加上滑动窗口
  - 只保留 system + 最近 N 轮（可配置）
  - 聊超过 N 轮后观察模型"失忆"现象
  - 加入 is_pinned 标记支持：pinned 消息在截断时强制保留

任务2：实现摘要压缩
  - 当历史超过阈值时，调用模型把旧对话总结为一段摘要
  - 摘要替代原始历史，放在 system 之后
  - 对比：丢弃 vs 摘要，模型是否还能记住早期信息

任务3：实现动态 System Prompt + Token 预算分配
  - 设计一个 system prompt 模板，动态注入关键上下文（用户偏好/当前任务等）
  - 调用前预估 token 数，自动决定截断还是压缩
...
> **本周内容：**
> 1. [Pydantic 数据模型（AI 工程的基石）](#1-pydantic-数据模型ai-工程的基石)  
> 2. [JSON Schema 规范](#2-json-schema-规范)  
> 3. [大模型 JSON Mode](#3-大模型-json-mode)  
> 4. [Structured Outputs（结构化输出 API）](#4-structured-outputs结构化输出-api)  
> 5. [输出校验与重试机制](#5-输出校验与重试机制)  
...
> **本周内容：**
> 1. [Function Calling 的通信流程](#1-function-calling-的通信流程)  
> 2. [工具描述的 JSON Schema 编写](#2-工具描述的-json-schema-编写)  
> 3. [手写工具调度器（Tool Dispatcher）](#3-手写工具调度器tool-dispatcher)  
> 4. [多轮工具调用与对话循环](#4-多轮工具调用与对话循环)  
> 5. [从 Function Calling 到 Agent 的认知跃迁](#5-从-function-calling-到-agent-的认知跃迁)  
...
> **本周内容：**
> 1. [Embedding 的本质与 API 调用](#1-embedding-的本质与-api-调用)  
> 2. [余弦相似度（半天搞定）](#2-余弦相似度半天搞定)  
> 3. [Embedding 模型选型实验](#3-embedding-模型选型实验)  
> 4. [纯内存语义搜索 Demo](#4-纯内存语义搜索-demo)  
> 5. [Embedding 的局限性认知](#5-embedding-的局限性认知)  
...
> **本周内容：**
> 1. [RAG 完整流程组装](#1-rag-完整流程组装)  
> 2. [Naive RAG 的典型缺陷](#2-naive-rag-的典型缺陷)  
> 3. [简单优化尝试](#3-简单优化尝试)  
...
> **本周内容：**
> 1. [LangChain 架构总览](#1-langchain-架构总览)  
> 2. [Prompt Templates 与 Output Parsers](#2-prompt-templates-与-output-parsers)  
> 3. [Retrievers 与 RAG Chain](#3-retrievers-与-rag-chain)  
> 4. [Memory 与对话链](#4-memory-与对话链)  
> 5. [框架的边界认知](#5-框架的边界认知)  
...
> **本周内容：**
> 1. [Query Rewrite（查询重写）](#1-query-rewrite查询重写)  
> 2. [Hybrid Search（混合检索）](#2-hybrid-search混合检索)  
> 3. [Reranking（重排序）](#3-reranking重排序)  
> 4. [完整 Advanced RAG Pipeline](#4-完整-advanced-rag-pipeline)  
...
> **本周内容：**
> 1. [Vision API（图片理解）](#1-vision-api图片理解)  
> 2. [Whisper 语音转文字](#2-whisper-语音转文字)  
> 3. [TTS（文字转语音）与多模态整合](#3-tts文字转语音与多模态整合)  
...
> **本周内容：**
> 1. [ReAct Agent 模式](#1-react-agent-模式)  
> 2. [自定义工具开发](#2-自定义工具开发)  
> 3. [MCP（Model Context Protocol）协议](#3-mcpmodel-context-protocol协议)  
> 4. [编写 MCP Server](#4-编写-mcp-server)  
> 5. [Agent + MCP 整合](#5-agent-mcp-整合)  
...
> **本周内容：**
> 1. [从 while 循环到状态图](#1-从-while-循环到状态图)  
> 2. [条件边与分支逻辑](#2-条件边与分支逻辑)  
> 3. [Human-in-the-Loop（人工介入）](#3-human-in-the-loop人工介入)  
> 4. [子图与模块化](#4-子图与模块化)  
> 5. [实战：构建一个多步骤 Agent](#5-实战构建一个多步骤-agent)  
...
> 2. [Trace 分析与问题定位](#2-trace-分析与问题定位)  
> 3. [成本监控与优化](#3-成本监控与优化)  
> 4. [Prompt 版本管理与实验](#4-prompt-版本管理与实验)  
...
> **本周内容：**
> 1. [Harness 的概念与设计](#1-harness-的概念与设计)  
> 2. [自纠错循环（Self-Correction）](#2-自纠错循环self-correction)  
> 3. [Guardrails（护栏）](#3-guardrails护栏)  
> 4. [超时与熔断](#4-超时与熔断)  
...
> **本周内容：**
> 1. [评估的必要性](#1-评估的必要性)  
> 2. [RAG 评估指标](#2-rag-评估指标)  
> 3. [Agent 评估](#3-agent-评估)  
> 4. [评估驱动的迭代优化](#4-评估驱动的迭代优化)  
> 5. [自定义评估指标](#5-自定义评估指标)  
...
> **本周内容：**
> 1. [FastAPI 基础](#1-fastapi-基础)  
> 2. [流式响应（SSE）](#2-流式响应sse)  
> 3. [WebSocket 实时通信](#3-websocket-实时通信)  
> 4. [生产级 API 工程](#4-生产级-api-工程)  
> 5. [Agent 服务化](#5-agent-服务化)  
